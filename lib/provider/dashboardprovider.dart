...
class DashboardProvider with ChangeNotifier {
  ...
  checklocation( double lat, double long)async
  {
    ...
    var uri= await Uri.parse('https://bsoe.meestdrive.in/api/employees/currentLocationSave');
    try{
...
if(result==true)
{
   ...
   print("dsdsfdfgfhgfh  ${result}  ");
      ...
      Position position = await Geolocator.getCurrentPosition(desiredAccuracy: LocationAccuracy.high);
      print("latLongsddd ${position.longitude} ${position.latitude}");
      await dbHelper.insertLocationData(position.latitude!, position.longitude!, DateTime.now().millisecondsSinceEpoch);

 print("dsdsfdfgfhgfh ${InternetConnectionChecker().hasConnection}  ${position.longitude} ${position.latitude} 1");
}
